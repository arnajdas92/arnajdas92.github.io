...
function isGameFinished()
{
	...
	if(allOk){
		...
		setTimeout(function(){alertify.alert("Congratulations!! You did it!")}, 1000);
		reveal = true;
	}
...
function cnf(){
	...
	alertify.set({ buttonReverse: true });
			alertify.confirm("Want to reveal a number?", function (e) {
				if (e) {
					arnaj = true;
					return arnaj;
					//alertify.success("arnaj");
				} else {
					arnaj = false;
					return arnaj;
					//alertify.error("arnaj");
				}
			});
			//return true;
...
{
	if(reveal==false){
		if(confirm("Want to reveal all numbers?")){
			for(var row=0;row<9;row++){
				for(var col=0;col<9;col++){
					var obj =document.getElementById('square_'+row+'_'+col);
					var spans = obj.getElementsByTagName('SPAN');
			...
			gameFinished=true;
			setTimeout(function(){alertify.alert("You finished the game by revealing all the numbers!")}, 1000);
		}
		
	}
	else
		alertify.alert("Numbers already revealed");
	reveal = true;
}
